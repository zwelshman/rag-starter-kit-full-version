...
in the provided documents."""
    ...
    def __init__(
        ...
        Args:
            ...
            vector_weight: Weight for semantic/vector search in hybrid mode
            bm25_weight: Weight for keyword/BM25 search in hybrid mode
        """
...
def create_pipeline(
    ...
    """
    Create a new RAG pipeline with configurable LLM provider and model.
    ...
    Args:
        ...
        chunk_overlap: Overlap between chunks
        n_results: Number of results to retrieve
        llm_provider: LLM provider name (anthropic, openai, cohere, ollama)
        llm_model: Model name for the provider
        semantic_weight: Weight for semantic search (0.0-1.0), BM25 weight = 1 - semantic_weight

    Returns:
        Initialized RAGPipeline
    """
    logger.info("=" * 60)
    logger.info("CREATING NEW RAG PIPELINE (PRO VERSION)")
    logger.info("=" * 60)
    ...
    logger.info(f"Semantic Weight: {semantic_weight} (BM25: {1 - semantic_weight})")
    ...
    logger.info(f"RAG Pipeline created successfully with {llm_provider}/{llm_model}")
    logger.info("=" * 60)
